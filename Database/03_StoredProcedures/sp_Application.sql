...
USE [UniversityERP]
GO
/****** Object:  StoredProcedure [dbo].[sp_Application]    Script Date: 8/17/2026 1:29:06 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
ALTER PROCEDURE [dbo].[sp_Application]
    ...
    -- Document Verification (Step 5)
    @SubmittedDocumentIds  NVARCHAR(MAX) = NULL,
    ...
    -- Registrar "All Students" overview filters (GETSTUDENTOVERVIEW)
    @FilterCourseId        INT = NULL,
    @FilterBranch          NVARCHAR(100) = NULL,
    @FilterSessionId       INT = NULL,
    @FilterSemester        NVARCHAR(20) = NULL,
...
BEGIN
    ...
    ELSE IF @Flag = 'GETDOCCHECKLIST'
    BEGIN
        DECLARE @AppCourseId INT;

        SELECT @AppCourseId = a.CourseId
        FROM Application ap
        INNER JOIN AdmissionSetup a ON a.AdmissionSetupId = ap.AdmissionSetupId
        WHERE ap.ApplicationId = @ApplicationId;

        SELECT DISTINCT de.DocumentEnclosureId, de.DocumentName, de.IsMandatory,
               ISNULL(ad.IsSubmitted, 0) AS IsSubmitted
        FROM RequiredDocumentMaster r
        INNER JOIN RequiredDocumentDetail rd ON rd.RequiredDocumentId = r.RequiredDocumentId
        INNER JOIN DocumentEnclosureMaster de ON de.DocumentEnclosureId = rd.DocumentEnclosureId
        LEFT JOIN ApplicationDocument ad
            ON ad.DocumentEnclosureId = de.DocumentEnclosureId AND ad.ApplicationId = @ApplicationId
        WHERE r.CourseId = @AppCourseId
          AND r.IsActive = 1
          AND de.IsActive = 1;
    END
    ...
    ELSE IF @Flag = 'VERIFYDOCS'
    BEGIN
        DECLARE @VCourseId INT;

        SELECT @VCourseId = a.CourseId
        FROM Application ap
        INNER JOIN AdmissionSetup a ON a.AdmissionSetupId = ap.AdmissionSetupId
        WHERE ap.ApplicationId = @ApplicationId;

        DELETE FROM ApplicationDocument WHERE ApplicationId = @ApplicationId;

        INSERT INTO ApplicationDocument (ApplicationId, DocumentEnclosureId, IsSubmitted)
        SELECT DISTINCT @ApplicationId, de.DocumentEnclosureId,
               CASE WHEN EXISTS (
                   SELECT 1 FROM STRING_SPLIT(@SubmittedDocumentIds, ',') s
                   WHERE TRY_CAST(s.value AS INT) = de.DocumentEnclosureId
               ) THEN 1 ELSE 0 END
        FROM RequiredDocumentMaster r
        INNER JOIN RequiredDocumentDetail rd ON rd.RequiredDocumentId = r.RequiredDocumentId
        INNER JOIN DocumentEnclosureMaster de ON de.DocumentEnclosureId = rd.DocumentEnclosureId
        WHERE r.CourseId = @VCourseId
          AND r.IsActive = 1
          AND de.IsActive = 1;
        ...
        IF EXISTS (
            ...
            LEFT JOIN ApplicationDocument ad
                ON ad.DocumentEnclosureId = de.DocumentEnclosureId AND ad.ApplicationId = @ApplicationId
            WHERE r.CourseId = @VCourseId AND r.IsActive = 1 AND de.IsActive = 1 AND de.IsMandatory = 1
              AND ISNULL(ad.IsSubmitted, 0) = 0
        )
            SET @AllMandatoryOk = 0;
    ...
    -- =========================================================
    -- REGISTRAR "ALL STUDENTS" OVERVIEW (list + filters)
    -- =========================================================
    ELSE IF @Flag = 'GETSTUDENTOVERVIEW'
    BEGIN
        SELECT
            ap.ApplicationId,
            ap.ApplicationNo,
            ...
            ap.Stage,
            ap.DocVerified,
            ISNULL(req.ReqCount, 0)  AS RequiredDocCount,
            ISNULL(sub.SubCount, 0)  AS SubmittedDocCount
        FROM Application ap
        INNER JOIN AdmissionSetup a   ON a.AdmissionSetupId = ap.AdmissionSetupId
        INNER JOIN CourseMaster c     ON c.CourseId = a.CourseId
        INNER JOIN AcademicSession s  ON s.AcademicSessionId = a.AcademicSessionId
        OUTER APPLY (
            SELECT COUNT(DISTINCT de.DocumentEnclosureId) AS ReqCount
            FROM RequiredDocumentMaster r
            INNER JOIN RequiredDocumentDetail rd ON rd.RequiredDocumentId = r.RequiredDocumentId
            INNER JOIN DocumentEnclosureMaster de ON de.DocumentEnclosureId = rd.DocumentEnclosureId
            WHERE r.CourseId = c.CourseId AND r.IsActive = 1 AND de.IsActive = 1
        ) req
        OUTER APPLY (
            SELECT COUNT(DISTINCT de2.DocumentEnclosureId) AS SubCount
            FROM RequiredDocumentMaster r2
            INNER JOIN RequiredDocumentDetail rd2 ON rd2.RequiredDocumentId = r2.RequiredDocumentId
            INNER JOIN DocumentEnclosureMaster de2 ON de2.DocumentEnclosureId = rd2.DocumentEnclosureId
            INNER JOIN ApplicationDocument ad2
                ON ad2.DocumentEnclosureId = de2.DocumentEnclosureId
                AND ad2.ApplicationId = ap.ApplicationId AND ad2.IsSubmitted = 1
            WHERE r2.CourseId = c.CourseId AND r2.IsActive = 1 AND de2.IsActive = 1
        ) sub
        WHERE (@FilterCourseId   IS NULL OR c.CourseId = @FilterCourseId)
          AND (@FilterBranch     IS NULL OR ap.Branch = @FilterBranch)
          AND (@FilterSessionId  IS NULL OR s.AcademicSessionId = @FilterSessionId)
          AND (@FilterSemester   IS NULL OR ap.Semester = @FilterSemester)
          AND (@SearchText IS NULL OR @SearchText = ''
               OR ap.FullName LIKE '%' + @SearchText + '%'
               OR ap.ApplicationNo LIKE '%' + @SearchText + '%'
               OR ap.RegistrationNumber LIKE '%' + @SearchText + '%'
               OR ap.Phone LIKE '%' + @SearchText + '%')
        ORDER BY ap.ApplicationId DESC;
    END

    -- =========================================================
    -- REGISTRAR : Submitted documents list (popup)
    -- =========================================================
    ELSE IF @Flag = 'GETSUBMITTEDDOCS'
    BEGIN
        DECLARE @SDCourseId INT;
        SELECT @SDCourseId = a.CourseId
        FROM Application ap
        INNER JOIN AdmissionSetup a ON a.AdmissionSetupId = ap.AdmissionSetupId
        WHERE ap.ApplicationId = @ApplicationId;
        ...
        INNER JOIN RequiredDocumentDetail rd ON rd.RequiredDocumentId = r.RequiredDocumentId
        INNER JOIN DocumentEnclosureMaster de ON de.DocumentEnclosureId = rd.DocumentEnclosureId
        INNER JOIN ApplicationDocument ad
            ON ad.DocumentEnclosureId = de.DocumentEnclosureId
            AND ad.ApplicationId = @ApplicationId AND ad.IsSubmitted = 1
        WHERE r.CourseId = @SDCourseId AND r.IsActive = 1 AND de.IsActive = 1;
    END

    -- =========================================================
    -- REGISTRAR : Full student profile (4-card view)
    -- =========================================================
    ELSE IF @Flag = 'GETSTUDENTPROFILE'
    BEGIN
        SELECT ap.ApplicationId, ap.ApplicationNo, ap.FullName, ap.Email, ap.Phone,
               c.CourseName, ap.Branch, ap.Semester, cat.CategoryName,
               ap.RegisteredOn, ap.Stage, ap.DocVerified,
               ISNULL(req.ReqCount, 0) AS RequiredDocCount,
               ISNULL(sub.SubCount, 0) AS SubmittedDocCount
        FROM Application ap
        INNER JOIN AdmissionSetup a ON a.AdmissionSetupId = ap.AdmissionSetupId
        ...
        SELECT ap.FeeAmount, ap.FeePaid, ap.FeeReceiptNo, ap.FeeMode, ap.FeePaymentDate,
               am.AdmissionModeName, ap.Stage AS AdmissionStatus
        FROM Application ap
        LEFT JOIN AdmissionModeMaster am ON am.AdmissionModeId = ap.AdmissionModeId
        WHERE ap.ApplicationId = @ApplicationId;
        ...
        LEFT JOIN ApplicationDocument ad
            ON ad.DocumentEnclosureId = de.DocumentEnclosureId AND ad.ApplicationId = @ApplicationId
        INNER JOIN Application ap2 ON ap2.ApplicationId = @ApplicationId
        INNER JOIN AdmissionSetup a2 ON a2.AdmissionSetupId = ap2.AdmissionSetupId
        WHERE r.CourseId = a2.CourseId AND r.IsActive = 1 AND de.IsActive = 1;
    END
END